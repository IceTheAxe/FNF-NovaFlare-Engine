		...
		if (doPrecache)
		{
			...
			while (true)
			{
				if (loader.checkLoaded())
				{
					...
					if (loader.loadThread != null) {
						loader.loadThread.minThreads = 0;
						loader.loadThread.cancel(); // kill all workers safely
					}
...
');
		...
		// Loading is a safe place to collect parser/decode temporaries. Disabling
		// GC here makes every completed worker and script AST accumulate until the
		// song starts, which is exactly where mobile memory peaks.
