		...
		FlxG.signals.preStateSwitch.add(function() {
			...
			if (Flags.MOD_REDIRECT_STATES.exists(stateName)) {
				@:privateAccess {
					...
					if (classFromString != null) FlxG.game._nextState = Type.createInstance(classFromString, []);
					else FlxG.game._nextState = new codename.funkin.backend.scripting.ModState(Flags.MOD_REDIRECT_STATES.get(stateName));
				}
			}
		});
