...
import codename.funkin.backend.assets.ZipFolderLibrary;
import codename.funkin.backend.chart.EventsData;
import codename.funkin.backend.system.framerate.Framerate;
import codename.funkin.editors.ModConfigWarning;
import codename.funkin.menus.TitleState;
import haxe.io.Path;
...
class MainState extends FlxState {
	...
	public override function create() {
		...
		#if sys
		var tempRoot = codenamechain.CodeNameMode.getTempRoot();
		CoolUtil.deleteFolder(tempRoot); // delete temp folder
		if (!FileSystem.exists(tempRoot)) FileSystem.createDirectory(tempRoot);
		#end
		Options.save();
		...
		inline function loadLib(path:String, name:String)
			Paths.assetsTree.addLibrary(ModsFolder.loadModLib(path, true, name));
		...
		#if GLOBAL_SCRIPT
		codename.funkin.backend.scripting.GlobalScript.destroy();
		#end
		...
		Flags.reset();
		Flags.load();
		codename.funkin.savedata.FunkinSave.init();
		...
		var startState:Class<FlxState> = Flags.DISABLE_WARNING_SCREEN ? TitleState : codename.funkin.menus.WarningState;
