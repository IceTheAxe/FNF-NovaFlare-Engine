...
#if MOD_SUPPORT
import codename.funkin.backend.utils.SysZip.SysZipEntry;
import codename.funkin.backend.utils.SysZip;
...
class ZipFolderLibrary extends AssetLibrary implements IModsAssetLibrary {
	...
	public function getVideoRemap(originalPath:String):String {
		...
		var newPath = '${codenamechain.CodeNameMode.getTempRoot()}/${_parsedAsset.length}-zipvideo-${_parsedAsset.split("/").pop()}';
