...
#if windows
import codename.funkin.backend.utils.NativeAPI.FileAttribute;
...
class AudioFixClient : public IMMNotificationClient {
	...
	HRESULT STDMETHODCALLTYPE OnDefaultDeviceChanged(
		...
		LPCWSTR pwstrDeviceId)
	{
		::codename::funkin::backend::_hx_system::Main_obj::audioDisconnected = true;
		return S_OK;
	};
};
...
final class Windows {
	...
	public static function registerAudio() {
		codename.funkin.backend.system.Main.audioDisconnected = false;
	}
	...
	// Reuse the terminal that launched the game (for example `lime test`) so
	// script errors stay in that command line. A normal double-click has no
	// parent console, in which case create a visible one instead.
