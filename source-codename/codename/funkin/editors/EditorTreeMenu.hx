...
class EditorTreeMenu extends codename.funkin.options.TreeMenu {
	...
	public inline function setBackgroundRotation(rotation:Float) {
		bg.angle = rotation;
		bg.velocity.set(85, 0).degrees = rotation;
	}
	...
	private function warnCompressLibrary() {
		...
		var compressedList = Paths.assetsTree.libraries.filter(l -> codename.funkin.backend.assets.AssetsLibraryList.getCleanLibrary(l).isCompressed);
		var modNameList = [for (l in compressedList) {
			l = codename.funkin.backend.assets.AssetsLibraryList.getCleanLibrary(l);
			if (l is codename.funkin.backend.assets.IModsAssetLibrary) cast(l, codename.funkin.backend.assets.IModsAssetLibrary).modName;
		}];
		warningMessage += modNameList.join(", ");
		var zipLibraryWarning = new codename.funkin.editors.ui.UIWarningSubstate("Compressed Library Detected!", warningMessage, [{label: "Ok", color: 0x969533, onClick: (state) -> {} }], false);
...
class EditorTreeMenuScreen extends codename.funkin.options.TreeMenuScreen {
		...
		?newButton:String, ?newButtonDesc:String, ?newCallback:Void->Void)
	{
		...
		if (newCallback != null) {
			insert(0, new codename.funkin.options.type.NewOption(getID(newButton), getID(newButtonDesc), newCallback));
			curSelected = 1;
		}
	}
}
