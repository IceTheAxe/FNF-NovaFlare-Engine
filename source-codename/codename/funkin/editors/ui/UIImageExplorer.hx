...
class UIImageExplorer extends UIFileExplorer {
	...
	public var selectedImageName:String = null;
	...
	public function uploadImage(filePath:String, file:Bytes) {
		...
		selectedImageName = isAtlas ? Path.withoutDirectory(directoryPath) : imagePath.file;

		var message = new StringBuf();
		if (isAtlas) message.add('${selectedImageName}/');
