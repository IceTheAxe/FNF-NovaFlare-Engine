...
import codename.funkin.editors.extra.PropertyButton;
...
import codename.funkin.editors.character.CharacterEditor;

using codename.funkin.backend.utils.BitmapUtil;
...
class CharacterInfoScreen extends UISubstateWindow {
	...
	public function saveCharacterInfo() {
		...
		if (defaultAimFPS.value != oldDefFPS) {
			var animButtonID:Int = 0;
			for (anim in character.getAnimOrder()) {
				var animData = character.animDatas.get(anim);
				if (animData.fps == oldDefFPS) {
					var animButton:CharacterAnimButton = CharacterEditor.instance.characterAnimsWindow.buttons.members[animButtonID];
					animButton.changeFPS(defaultAimFPS.value);
				}
				animButtonID++;
			}
		}
