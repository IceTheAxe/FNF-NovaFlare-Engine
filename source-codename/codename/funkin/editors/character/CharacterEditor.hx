package codename.funkin.editors.character;

import codename.funkin.editors.character.CharacterInfoScreen.CharacterExtraInfo;
import codename.funkin.editors.extra.AxisGizmo;
import flixel.math.FlxRect;
import codename.funkin.editors.stage.StageEditor;
import codename.funkin.game.Stage;
import codename.funkin.editors.ui.UITopMenu.UITopMenuButton;
import codename.funkin.editors.extra.CameraHoverDummy;
import openfl.display.BitmapData;
import flixel.math.FlxPoint;
import codename.funkin.backend.system.framerate.Framerate;
import codename.funkin.backend.utils.XMLUtil.AnimData;
import codename.funkin.editors.ui.UIContextMenu.UIContextMenuOption;
import codename.funkin.editors.ui.UIContextMenu.UIContextMenuOptionSpr;
import codename.funkin.game.Character;
import haxe.xml.Access;
import haxe.xml.Printer;
import codename.funkin.editors.ui.UIImageExplorer.ImageSaveData;
...
class CharacterEditor extends UIState {
	...
	function _undo(undo:CharacterEditorChange) {
		...
		switch (undo) {
			...
			case CCharEditSprite(fileID):
				var cneisdPath:String = '${codenamechain.CodeNameMode.getTempRoot()}/__undo__${Type.getClassName(Type.getClass(FlxG.state))}__${fileID}.cneisd';
	...
	function _redo(redo:CharacterEditorChange) {
		...
		switch (redo) {
			...
			case CCharEditSprite(fileID):
				var cneisdPath:String = '${codenamechain.CodeNameMode.getTempRoot()}/__redo__${Type.getClassName(Type.getClass(FlxG.state))}__${fileID}.cneisd';
				if (FileSystem.exists(cneisdPath)) {
					try {
						var cneisdData:String = File.getContent(cneisdPath);
						var imageSaveData:ImageSaveData = UIImageExplorer.deserializeSaveDataGlobal(cneisdData);
