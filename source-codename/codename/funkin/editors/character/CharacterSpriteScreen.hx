...
class CharacterSpriteScreen extends UISubstateWindow {
	...
	public inline function addToUndo() {
		...
		CoolUtil.safeSaveFile('${codenamechain.CodeNameMode.getTempRoot()}/__undo__${Type.getClassName(Type.getClass(FlxG.state))}__${idCounter}.cneisd', UIImageExplorer.serializeSaveDataGlobal(ogImageSaveData));
		CoolUtil.safeSaveFile('${codenamechain.CodeNameMode.getTempRoot()}/__redo__${Type.getClassName(Type.getClass(FlxG.state))}__${idCounter}.cneisd', UIImageExplorer.serializeSaveDataGlobal(imageExplorer.getSaveData()));
	}
}
