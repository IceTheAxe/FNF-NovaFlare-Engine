...
import codename.funkin.backend.scripting.events.healthicon.HealthIconChangeEvent;
	...
	override function update(elapsed:Float):Void {
		...
		if (animation.curAnim != null || this.animated) {
			...
			if (data.isValid && curAnimState != localAnimState) {
				...
				codename.funkin.backend.scripting.GlobalScript.event("onHealthIconAnimChange", event);
				if (!event.cancelled) {
					if (this.animated) {
...
/**
 * Used for `codename.funkin.game.HealthIcon.sprTrackerAlignment`.
