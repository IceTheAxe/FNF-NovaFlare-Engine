package codename.funkin.game.cutscenes;

import flixel.sound.FlxSound;
import codename.funkin.backend.scripting.Script;
import codename.funkin.backend.scripting.events.CancellableEvent;
import codename.funkin.backend.scripting.events.DynamicEvent;
import codename.funkin.backend.scripting.events.dialogue.*;
import codename.funkin.backend.utils.FunkinParentDisabler;
import codename.funkin.game.cutscenes.dialogue.*;
import haxe.io.Path;
...
class DialogueCutscene extends ScriptedCutscene {
	...
	public override function update(elapsed:Float) {
		...
		if(controls.ACCEPT #if TOUCH_CONTROLS || (mobile.funkin.backend.utils.TouchUtil.justPressed && !controls.BACK && !controls.PAUSE) #end) {
