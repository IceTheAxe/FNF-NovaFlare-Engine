...
import codename.funkin.backend.scripting.events.CancellableEvent;
import codename.funkin.backend.scripting.events.character.*;
import codename.funkin.backend.scripting.events.sprite.*;
import codename.funkin.backend.scripting.events.PointEvent;
import codename.funkin.backend.scripting.events.DrawEvent;
import codename.funkin.backend.system.Conductor;
import codename.funkin.backend.system.interfaces.IBeatReceiver;
import codename.funkin.backend.system.interfaces.IOffsetCompatible;
import codename.funkin.backend.utils.MatrixUtil;
import codename.funkin.backend.utils.XMLUtil;
...
@:allow(codename.funkin.desktop.editors.CharacterEditor)
@:allow(codename.funkin.game.StrumLine)
@:allow(codename.funkin.game.PlayState)
	...
	*/
	...
	public static function getList(?mods:Bool = false, includeFolders:Bool = false, folder:String = 'data/characters/'):Array<String> {
		...
		for (path in Paths.getFolderContent(folder, true, mods ? MODS : BOTH))
			if (Path.extension(path).toLowerCase() == "xml")
