...
@:allow(codename.funkin.backend.assets.ModsFolder)
...
class TitleState extends MusicBeatState
{
	...
	function goToMainMenu(force = true) {
		...
		if (!force && !Flags.DISABLE_AUTOUPDATER) {
			codename.funkin.backend.system.updating.UpdateUtil.waitForUpdates(false, (report) -> {
				hasCheckedUpdates = true;
				if (FlxG.state != this) return;

				if (!report.newUpdate) goToMainMenu(true);
				else FlxG.switchState(new codename.funkin.backend.system.updating.UpdateAvailableScreen(report));
			}, true);
		}
		else
		#end {
