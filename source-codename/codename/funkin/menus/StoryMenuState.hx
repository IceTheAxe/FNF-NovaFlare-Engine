...
import codename.funkin.backend.scripting.events.menu.storymenu.*;
import codename.funkin.backend.week.*;
...
class StoryMenuState extends MusicBeatState {
	...
	public override function update(elapsed:Float) {
		...
		if (canSelect) {
			...
			changeWeek((controls.UP_P ? -1 : 0) + (controls.DOWN_P ? 1 : 0) - scroll);
...
class StoryWeeklist {
	...
	public function getWeeksFromSource(source:codename.funkin.backend.assets.AssetSource, useTxt:Bool = true, loadCharactersData:Bool = true) {
