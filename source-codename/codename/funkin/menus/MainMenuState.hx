package codename.funkin.menus;

import flixel.FlxState;
import flixel.effects.FlxFlicker;
import flixel.tweens.FlxTween;
import codename.funkin.backend.FunkinText;
import codename.funkin.backend.scripting.events.menu.MenuChangeEvent;
import codename.funkin.backend.scripting.events.NameEvent;
import codename.funkin.menus.credits.CreditsMain;
import codename.funkin.options.OptionsMenu;
		...
		if (!selectedSomethin)
		{
			if (canAccessDebugMenus) {
				if (controls.DEV_ACCESS) {
					...
					persistentDraw = true;
					openSubState(new codename.funkin.editors.EditorPicker());
				}
				/*
				if (FlxG.keys.justPressed.SEVEN)
					FlxG.switchState(new codename.funkin.desktop.DesktopMain());
				if (FlxG.keys.justPressed.EIGHT) {
					CoolUtil.safeSaveFile("chart.json", Json.stringify(codename.funkin.backend.chart.Chart.parse("dadbattle", "hard")));
				}
				*/
			}
